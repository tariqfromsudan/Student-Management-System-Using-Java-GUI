...
class App(tk.Tk):
    ...
    def _build_students_tab(self):
        ...
        for c, txt, w, anchor in [("id","Student ID",120,"w"),("name","Name",200,"w"),("dept","Department",120,"w"),("gender","Gender",70,"center"),("year","Year",60,"center"),("gpa","GPA",60,"center")]:
    ...
    def _add_student(self):
        try:
            ...
            s = Student(student_id=sid, name=self.name_var.get().strip() or "Unnamed", department=self.dept_var.get().strip() or "-", gender=self.gender_var.get().strip() or "F", year=int(self.year_var.get()))
    ...
    def _save(self):
        try:
            self.reg.save(); messagebox.showinfo("Saved", "Data + catalog saved.")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _build_academics_tab(self):
        ...
        ttk.Entry(frm, textvariable=self.subj_var, width=14).grid(row=0, column=3, padx=6, pady=6)
        ...
        ttk.Entry(att, textvariable=self.date_var, width=14).grid(row=0, column=5, padx=6, pady=6)
    ...
    def _enroll_subject(self):
        ...
        if not sid or not subj:
            messagebox.showwarning("Input", "Enter Student ID and Subject Code."); return
        s = self._get_student_or_warn(sid)
        if not s: return
        try:
            s.enroll_subject(subj); messagebox.showinfo("Enrolled", f"{sid} -> {subj}")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _drop_subject(self):
        ...
        if not s: return
        try:
            s.drop_subject(subj); messagebox.showinfo("Dropped", f"{sid} -/-> {subj}")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _add_grade(self):
        ...
        try:
            ...
            messagebox.showwarning("Score", "Enter a valid number (0..100)"); return
        try:
            s.add_grade(subj, score); messagebox.showinfo("Grade", f"Added {score} to {sid}/{subj}")
        except Exception as e: messagebox.showerror("Error", str(e))

    def _undo_grade(self):
        sid = self.sid2_var.get().strip()
        if not sid:
            messagebox.showwarning("Input", "Enter Student ID."); return
        s = self._get_student_or_warn(sid)
        if not s: return
        try:
            s.undo_last_grade(); messagebox.showinfo("Undo", "Last grade undone.")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _process_attendance(self):
        try:
            cnt = self.reg.process_attendance(); messagebox.showinfo("Processed", f"Processed {cnt} attendance record(s).")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _build_bonus_tab(self):
        ...
        ttk.Entry(reps, textvariable=self.alpha_var, width=6).grid(row=0, column=1, padx=6, pady=6)
        ttk.Label(reps, text="Weight β (Attendance)").grid(row=0, column=2, padx=6, pady=6, sticky="w")
        ttk.Entry(reps, textvariable=self.beta_var, width=6).grid(row=0, column=3, padx=6, pady=6)
        ttk.Label(reps, text="Top per Department").grid(row=0, column=4, padx=6, pady=6, sticky="w")
        ttk.Spinbox(reps, from_=1, to=5, textvariable=self.topk_var, width=5).grid(row=0, column=5, padx=6, pady=6)
        ...
        editor = ttk.Frame(tt); editor.pack(side=tk.TOP, fill=tk.X, padx=6, pady=6)
        self.slot_code = tk.StringVar(); self.slot_start = tk.StringVar(); self.slot_end = tk.StringVar(); self.slot_weight = tk.StringVar(value="1")
        ttk.Label(editor, text="Subject Code").grid(row=0, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(editor, textvariable=self.slot_code, width=10).grid(row=0, column=1, padx=4, pady=4)
        ttk.Label(editor, text="Start (HH:MM)").grid(row=0, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(editor, textvariable=self.slot_start, width=8).grid(row=0, column=3, padx=4, pady=4)
        ttk.Label(editor, text="End (HH:MM)").grid(row=0, column=4, padx=4, pady=4, sticky="w")
        ttk.Entry(editor, textvariable=self.slot_end, width=8).grid(row=0, column=5, padx=4, pady=4)
        ttk.Label(editor, text="Weight").grid(row=0, column=6, padx=4, pady=4, sticky="w")
        ...
        for c, txt, w, anchor in [("code","Subject",100,"w"),("start","Start",80,"center"),("end","End",80,"center"),("weight","Weight",70,"center")]:
            self.catalog_tree.heading(c, text=txt); self.catalog_tree.column(c, width=w, anchor=anchor)
        self.catalog_tree.pack(side=tk.TOP, fill=tk.X, padx=6, pady=6)

        # Optimizer
        opt = ttk.Frame(tt); opt.pack(side=tk.TOP, fill=tk.X, padx=6, pady=6)
        self.opt_sid = tk.StringVar()
        ttk.Label(opt, text="Student ID").pack(side=tk.LEFT)
        ttk.Entry(opt, textvariable=self.opt_sid, width=12).pack(side=tk.LEFT, padx=6)
    ...
    def _optimize_timetable(self):
        ...
        if not sid:
            messagebox.showwarning("Input", "Enter a Student ID."); return
        try:
